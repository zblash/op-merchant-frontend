...
function ObligationsPage(props: React.PropsWithChildren<ObligationsPageProps>) {
  ...
  const [
    allObligationsPageNumber,
    setAllObligationsPageNumber,
  ] = React.useState(1);
  const {
  ...
  const TABLE_DATA_COLUMNS = React.useMemo<
    UITableColumns<IObligationActivityResponse>[]
  >(() => {
    return [
      {
        title: "Belge No",
    ...
    <Container>
      <StyledPageContainer>
        ...
        <UITable
          ...
          rowCount={
            allObligations.elementCountOfPage > 0
              ? allObligations.elementCountOfPage
              : 5
          }
